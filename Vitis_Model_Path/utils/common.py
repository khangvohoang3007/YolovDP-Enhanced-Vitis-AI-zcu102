...
class Decoupled_Detect(nn.Module):
    ...
    export = False  # export mode (Set True for DPU/Inference)
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # ModuleList chứa các Head đã được Decoupled
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            # 1. Chạy phần tích chập (Convolution) trên DPU
            # Đây là phần nặng nhất, DPU sẽ đảm nhiệm 100%
            x[i] = self.m[i](x[i])  
            
            # 2. KIỂM TRA CHẾ ĐỘ EXPORT (Dành cho Vitis AI / Board)
            if self.export:
                # Trả về kết quả thô (raw) ngay lập tức
                # Tránh các phép toán view, permute, sigmoid gây Segfault trên DPU
                z.append(x[i])
                continue 
            
            # 3. Logic dành cho Training hoặc Inference trên PC (CPU/GPU)
            bs, _, ny, nx = x[i].shape
            # Reshape và Hoán vị channel để tính toán Box
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            
            if not self.training:  # Chế độ inference thông thường trên PC
                if self.grid[i].shape[2:4] != x[i].shape[2:4] or self.onnx_dynamic:
                    self.grid[i], self.anchor_grid[i] = self._make_grid(nx, ny, i)

                y = x[i].sigmoid() # Sigmoid này chỉ chạy trên PC, không chạy trên Board
                if self.inplace:
                    y[..., 0:2] = (y[..., 0:2] * 2 + self.grid[i]) * self.stride[i]  # xy
                    y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                else:  # cho các thiết bị không hỗ trợ inplace
        ...
        return z if self.export else (torch.cat(z, 1) if not self.training else x)
...
class Conv(nn.Module):
    ...
    default_act = nn.ReLU(inplace=True)  # Updated for Vitis AI
...
# 1. Sửa FAMBlock để tránh chia tách Subgraph
class FAMBlock(nn.Module):
    ...
    def forward(self, x):
        # DPU hỗ trợ AdaptiveAvgPool2d về 1x1 rất tốt
        y = self.avg_pool(x)
        y = self.conv(y)
        return x * y
...
class LSKmodule(nn.Module):
    def __init__(self, c1, dim):
        ...
        # Nhánh 1: Conv kernel lớn (tách nhóm để tiết kiệm tham số, DPU hỗ trợ tốt)
        ...
        # Nhánh 2: GhostConv thay cho Conv thông thường để tăng tốc
        self.conv0_s = GhostConv(dim, dim // 2, 1)
        self.conv1_s = GhostConv(dim, dim // 2, 1)
        
        # GIẢI PHÁP TRÁNH SEGFAULT: Thay mean/max bằng Conv nén channel
        # Giảm từ 'dim' channels xuống 2 channels để làm map attention
        self.reducer = nn.Conv2d(dim, 2, 1) 
        
        self.conv_squeeze = nn.Conv2d(2, 2, 7, padding=3)
        self.conv_m = GhostConv(dim // 2, dim, 1)
        
        # DPU cực thích ReLU, tránh Sigmoid giữa chừng để không bị tách Subgraph
        self.act = nn.ReLU(inplace=True) 
    ...
    def forward(self, x):
        ...
        # 1. Ghép tensor: DPU hỗ trợ tốt
        attn_cat = torch.cat([attn1, attn2], dim=1)
        
        # 2. Thay cho Mean/Max (Tránh lỗi phân mảnh bộ nhớ)
        agg = self.reducer(attn_cat) 
        
        # 3. Activation an toàn cho DPU
        sig = self.act(self.conv_squeeze(agg))
        
        # 4. TRÁNH INDEXING (sig[:, 0]): 
        # Chúng ta cộng hai nhánh lại rồi nhân với sig để giữ tensor nguyên khối
        # Đây là cách an toàn nhất để không bị Segfault khi compile
        attn = self.conv_m((attn1 + attn2) * sig)
