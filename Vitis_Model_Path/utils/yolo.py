...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.m[i](x[i])  # Conv chạy trên DPU
            
            # Sửa lại ở đây: Nếu export thì append vào z rồi trả về luôn sau vòng lặp
            if self.export:
                z.append(x[i])
                continue

            # Các logic grid/sigmoid bên dưới chỉ dùng cho Train/Inference PC
            bs, _, ny, nx = x[i].shape
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            # ... (giữ nguyên phần tạo grid và sigmoid bên dưới)
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        ...
        self.nm = nm  # number of masksGhostConv
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    # 1. Ép mặc định ReLU cho toàn bộ Conv để DPU không bị tách Subgraph
    Conv.default_act = nn.ReLU(inplace=True)
    if act:
        LOGGER.info(f"{colorstr('activation:')} {act} (Overridden to ReLU for DPU compatibility)")
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # Nhánh 1: Các lớp tính toán (Conv, C3, Bottleneck...)
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, 
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.Conv2d):
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C3, C3TR, C3Ghost, C3SPP):
                args.insert(2, n)  # number of repeats
                n = 1
            
            # ÉP RELU: Đảm bảo các tham số activation trong args được thay bằng ReLU
            if m in (Conv, GhostConv, DWConv) and len(args) > 5:
                args[5] = nn.ReLU(inplace=True)
            elif m in (C3, C3Ghost, BottleneckCSP, C3TR, C3SPP) and len(args) > 4:
                args[4] = nn.ReLU(inplace=True)
        ...
        # Nhánh 2: Các lớp Head (Detect, Decoupled_Detect) - PHẢI TÁCH RIÊNG ELIF
